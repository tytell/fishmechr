...
1. Compute a center location. Ideally this is the center of mass, but it could be approximated in a variety of ways.
2. Compute the body curvature or excursion relative to a central axis, or both.
3. Compute the arc length along the curve of the body.
4. If the points are not consistently at the same arc length, interpolate the points so that each one is at a consistent point on the body.
...
$$ x_{com} = \tfrac{1}{4} \sum_{i=1}^{n-1} \left[ \Delta s_i \left( x_i + x_{i+1} \right) \left( w_i + w_{i+1} \right) \right] $$
$$ y_{com} = \tfrac{1}{4} \sum_{i=1}^{n-1} \left[ \Delta s_i \left( y_i + y_{i+1} \right) \left( w_i + w_{i+1} \right) \right] $$
...
```{r}
lampreydata |> 
  get_center_df(t, arclen, mxmm,mymm, width)
```
