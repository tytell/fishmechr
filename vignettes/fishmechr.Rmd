...
lampreydata <- lampreydata |> 
  interpolate_points_df(arclen, mxmm, mymm, spar = 0.2,
...
lampreydata <-
  lampreydata |> 
  get_center_df(arclen, mxmm_s,mymm_s, width=width)
...
lampreydata <-
  lampreydata |> 
  group_by(frame) |> 
  mutate(curve_ang = curvature(arclen_s, mxmm_s, mymm_s),
         curve_xy = curvature(arclen_s, mxmm_s, mymm_s, method="xy"))
```
...
lampreydata |> 
  filter(frame %in% c(10, 20)) |> 
  mutate(frame = factor(frame)) |> 
  ggplot(aes(x = arclen_s, color = frame, group = frame)) +
