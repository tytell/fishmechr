...
where $A(s)$ is the wave amplitude, $k$ is the wave length, $f$ is the oscillation frequency, and $t$ is time. In this case, the phase of the oscillation $\phi(s, t)$ at a particular point along the body is approximately equal to the argument of the cosine, $\phi(s, t) = 2\pi \left[ s / \lambda - f t \right] $. If the amplitude $A$ varies over quickly over time or space, this approximation may not be exactly correct.
