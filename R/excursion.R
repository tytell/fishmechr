...
get_primary_swimming_axis <- function(x, y, center = TRUE, na.rm = FALSE) {
  if (all(is.finite(x)) && all(is.finite(y))) {
    if (center) {
      x_ctr <- mean(x, na.rm = na.rm)
      y_ctr <- mean(y, na.rm = na.rm)
...
#' @param tm Column containing the time data. If a cutoff frequency is passed in,
...
#' * exc,exc_x: The new midlines centered and projected on to the swimming direction
#'   and the perpendicular axis. `exc` is useful as the lateral excursion of the
#'   swimming undulation.
#'
#' @concept pipeline
#' @export
get_primary_swimming_axis_df <- function(
  ...
  .out <- check.out(
    ...
    .out,
    .out_default = c(
      swimaxis_x = 'swimaxis_x',
      swimaxis_y = 'swimaxis_y',
      exc_x = 'exc_x',
      exc = 'exc'
    ),
    overwrite = overwrite
  ...
  # TODO: Make this work on a grouped data frame...
  ...
  if (check_reasonableness) {
    ...
    if (
      any(centering$notcenterx > 0.1) ||
        any(centering$notcentery > 0.1)
    ) {
      warning(
        "Many frames seem not to be centered around zero. Did you remember to subtract the center of mass?"
      )
    }
  }
